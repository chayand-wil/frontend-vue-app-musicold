...
import { fetchArtists, createArtist, updateArtist, deleteArtist } from './../../services/artist'
...
const saveArtist = async () => {
  try {
    if (modalMode.value === 'create') {
      await createArtist(currentArtist.value);
    } else {
      await updateArtist(currentArtist.value.id, currentArtist.value);
    }
    loadArtists();
    closeModal();
  } catch (error) {
